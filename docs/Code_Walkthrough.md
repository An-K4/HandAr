...
> **Cập nhật lần cuối tại commit `347b05f`** (kèm một số thay đổi chưa commit tại thời điểm viết
> — xem `git status`/`git diff` nếu cần biết chính xác). **Note cho agent:** file này bám theo TỪNG
