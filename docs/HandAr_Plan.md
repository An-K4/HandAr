...
## 9. Phase N — Dọn nợ của L/M và cấu trúc lại dự án ✅ ĐÃ HOÀN THÀNH

> **Lệch so với kế hoạch:**
>
> 1. **N1 phát hiện thêm 1 bug ngoài phạm vi ban đầu**: `AnimatedGifVisual` từng hoãn
>    `start()`/`stop()` tới `renderToBuffer()` (cờ `desiredActive`), khiến GIF không dừng giải mã
>    khi state hết khớp — chuyển thẳng vào `setActive()` (xem thêm mục 13 ở Phase M).
> 2. **N1 phát hiện 1 bug thật trong lúc tự code**: `ImageBackgroundRenderer` so sai biến
>    (so `canvas.width` với `lastH` thay vì `lastW`, và không gán lại `lastW`/`lastH`) và
>    `canvas.drawBitmap` bị đặt lọt vào trong khối `if` cache — nếu không phát hiện, nền sẽ
>    biến mất hoàn toàn sau frame đầu tiên ngay khi cache hoạt động đúng (bug im lặng, không
>    crash).
> 3. **N3 tổ chức `effect/visual/canvas/` sâu hơn đề xuất gốc** — tách thêm 2 thư mục con
>    `drawcanvas/` (nhóm vẽ tay: `StrokeVisual`, `StrokeModel`, `ClearOnActivate`,
>    `SkeletonOnlyVisual`, `HandSkeleton`) và `gojo/` (`GojoVisual`, `GojoModel`) thay vì để phẳng
>    chung 1 thư mục như bản gốc đề xuất.
> 4. **N3-7 (tách `EffectRepository.kt`) chỉ tách đúng 6 hiệu ứng** nhiều state/nhiều cấu hình
>    (`canvas_draw`, `earth`, `black_hole`, `gojo`, `test_effect_background`, `camera_shutter`)
>    vào `effect/catalog/`; 9 hiệu ứng 2-state đơn giản giữ nguyên khai thẳng trong
>    `EffectRepository.kt` — đúng tinh thần "chỉ tách cái phức tạp", không tách hết 15 hiệu ứng.
...
| Xoá / chia sẻ / **đổi tên** ngay trong màn thư viện | ✅ Xong (ở `VideoPlayerFragment`) | Đúng dự tính: không làm ở màn thư viện mà chuyển hẳn sang `VideoPlayerFragment` (menu 3 chấm). Xóa dùng lại `ConfirmDialog` nguyên bản từ `RecordedPreviewFragment` rồi `File.delete()` + `popBackStack()`. Đổi tên là UI mới hoàn toàn: `RenameDialog`/`dialog_rename.xml` (viền `border_effect_decorative_rectangle` bọc `EditText`, nền trong suốt) — **bắt buộc kiểm tra `newFile.exists()` TRƯỚC `File.renameTo()`**, vì Android ghi đè âm thầm khi trùng tên, không tự báo lỗi (xem `Code_Walkthrough.md` mục 12). Chia sẻ điều hướng sang `shareFragment` có sẵn (dùng lại từ `recordedPreviewFragment`), thêm argument `fromRecordedPreview: Boolean` (mặc định `false`) để ẩn nút Try Again khi không có `effectId` gốc |
